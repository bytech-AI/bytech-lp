...
(function(){var maxStep=1,submitted=false,slotSelected=false;function tk(type,data){data=data||{};data.route_id=ROUTE_ID;data.ref=(window.btRefInfo?window.btRefInfo().ref:'');data.lp_query=(window.btRefInfo?window.btRefInfo().q:'');data.source=SOURCE;data.entry='calendar';data.ts=new Date().toISOString();var img=new Image();img.src=GAS_URL+'?action=track&type='+encodeURIComponent(type)+'&id='+encodeURIComponent(data.email||ROUTE_ID)+'&route_id='+encodeURIComponent(ROUTE_ID)+'&data='+encodeURIComponent(JSON.stringify(data));}
